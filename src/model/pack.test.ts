...
describe("what must load", () => {
  ...
  it.each([
    ...
    "tables",
    "extends",
  ])("treats %s as optional on its own", (array) => {
    expect(parsePack(envelope({ [array]: undefined })).ok).toBe(true);
  });

  it.each(["author", "description"])(
    "accepts %s absent, null, or written",
    (field) => {
  ...
  it.each(["1.2.0", "1.0.0-beta.1", "2026.09", "0.1", "3"])(
    "accepts version %s",
    (version) => {
  ...
  it("accepts the shortest and longest pack id", () => {
    expect(parsePack(envelope({ id: "a".repeat(PACK_ID_MIN_LENGTH) })).ok).toBe(
      true,
    );
    expect(parsePack(envelope({ id: "a".repeat(PACK_ID_MAX_LENGTH) })).ok).toBe(
      true,
    );
  });
});
...
describe("what must be refused", () => {
  ...
  it("refuses unrelated JSON on its first field", () => {
    ...
    expect(result.problems.some((problem) => problem.path === "format")).toBe(
      true,
    );
  });
  ...
  it.each([null, undefined, "a pack", 42, []])(
    "refuses %s as a whole pack",
    (input) => {
  ...
  it.each([
    ...
    expect(parsePack(envelope({ [field]: "a".repeat(max + 1) })).ok).toBe(
      false,
    );
  });
  ...
  it.each(["classes", "ancestries", "spells", "items", "talents", "tables"])(
    ...
    (array) => {
      ...
      expect(
        parsePack(
          envelope({ [array]: entries.slice(0, MAX_ENTRIES_PER_ARRAY) }),
        ).ok,
      ).toBe(true);
    },
  );
  ...
  it("caps extends at its own constant", () => {
    ...
    expect(
      parsePack(
        envelope({ extends: extensions.slice(0, MAX_EXTENDS_PER_PACK) }),
      ).ok,
    ).toBe(true);
  });
...
describe("the leaves every entry is built from", () => {
  it("reports an id inside a pack bare, never namespaced", () => {
    ...
    expect(EntryId.safeParse("a".repeat(ENTRY_ID_MAX_LENGTH + 1)).success).toBe(
      false,
    );
  });
  ...
  it("leaves text optional and bounded wherever it appears", () => {
    ...
    expect(EntryText.safeParse("t".repeat(MAX_TEXT_LENGTH + 1)).success).toBe(
      false,
    );
  });
  ...
  it("requires a name and bounds it", () => {
    ...
    expect(EntryName.safeParse("n".repeat(MAX_NAME_LENGTH + 1)).success).toBe(
      false,
    );
  });
});
...
describe("the report", () => {
  ...
  it("says every shape a value could have taken, on one line", () => {
    const tables = [
      { ...TABLE, rows: [{ roll: "seven", text: "A rimeblade" }] },
    ];
...
describe("spells", () => {
  ...
  it.each(["id", "name", "tier", "classes", "range", "duration"])(
    ...
    (field) => {
      expect(
        SpellEntry.safeParse(entry(SPELL, { [field]: undefined })).success,
      ).toBe(false);
    },
  );
  ...
  it.each(["text", "page"])("leaves %s absent or null", (field) => {
    ...
    expect(SpellEntry.safeParse(entry(SPELL, { [field]: null })).success).toBe(
      true,
    );
  });
  ...
  it("matches range and duration exactly", () => {
    ...
    expect(
      SpellEntry.safeParse(entry(SPELL, { range: "medium" })).success,
    ).toBe(false);
    expect(SpellEntry.safeParse(entry(SPELL, { range: "Near" })).success).toBe(
      false,
    );
    expect(
      SpellEntry.safeParse(entry(SPELL, { duration: "concentration" })).success,
    ).toBe(false);
    expect(
      SpellEntry.safeParse(entry(SPELL, { duration: "permanent" })).success,
    ).toBe(true);
  });

  it("names classes bare within a pack and in full across packs", () => {
    expect(
      SpellEntry.safeParse(entry(SPELL, { classes: ["wizard"] })).success,
    ).toBe(true);
    expect(
      SpellEntry.safeParse(entry(SPELL, { classes: ["core:class:wizard"] }))
        .success,
    ).toBe(true);
    expect(
      SpellEntry.safeParse(entry(SPELL, { classes: ["Wizard"] })).success,
  ...
  it("caps the class list and allows it to be empty", () => {
    ...
    expect(SpellEntry.safeParse(entry(SPELL, { classes: many })).success).toBe(
      false,
    );
    expect(SpellEntry.safeParse(entry(SPELL, { classes: [] })).success).toBe(
      true,
    );
  });
  ...
  it("refuses an unknown key on a spell", () => {
    expect(
      SpellEntry.safeParse(entry(SPELL, { grants: "+1 str" })).success,
    ).toBe(false);
  });
});
...
describe("items", () => {
  ...
  it.each(["id", "name", "slots", "cost"])(
    "refuses an item missing %s",
    (field) => {
      expect(
        ItemEntry.safeParse(entry(ITEM, { [field]: undefined })).success,
      ).toBe(false);
    },
  );
  ...
  it("lets a thing be weightless and caps what one of it costs to carry", () => {
    ...
    expect(
      ItemEntry.safeParse(entry(ITEM, { slots: MAX_PACK_ITEM_SLOTS })).success,
    ).toBe(true);
    expect(
      ItemEntry.safeParse(entry(ITEM, { slots: MAX_PACK_ITEM_SLOTS + 1 }))
        .success,
    ).toBe(false);
    expect(ItemEntry.safeParse(entry(ITEM, { slots: -1 })).success).toBe(false);
    expect(ItemEntry.safeParse(entry(ITEM, { slots: 0.5 })).success).toBe(
      false,
    );
  });
  ...
  it("quotes a cost in one of the three coins", () => {
    for (const currency of ["gp", "sp", "cp"]) {
      expect(
        ItemEntry.safeParse(entry(ITEM, { cost: { amount: 1, currency } }))
          .success,
      ).toBe(true);
    }
    expect(
      ItemEntry.safeParse(entry(ITEM, { cost: { amount: 1, currency: "ep" } }))
        .success,
    ).toBe(false);
    expect(
      ItemEntry.safeParse(entry(ITEM, { cost: { amount: 0, currency: "gp" } }))
        .success,
    ).toBe(true);
    expect(
      ItemEntry.safeParse(
        entry(ITEM, { cost: { amount: MAX_COIN + 1, currency: "gp" } }),
      ).success,
    ).toBe(false);
    expect(ItemEntry.safeParse(entry(ITEM, { cost: "90gp" })).success).toBe(
      false,
    );
  });

  it.each(["weapon", "armor"])(
    "leaves the %s block absent or null",
    (block) => {
      expect(
        ItemEntry.safeParse(entry(ITEM, { [block]: undefined })).success,
      ).toBe(true);
      expect(ItemEntry.safeParse(entry(ITEM, { [block]: null })).success).toBe(
        true,
      );
    },
  );
  ...
  it("reads the armour block DATA-MODEL.md §4 shows", () => {
    ...
    expect(
      ItemEntry.safeParse(entry(ITEM, { weapon: null, armor })).success,
    ).toBe(true);
    expect(
      ItemEntry.safeParse(
        entry(ITEM, { armor: { ...armor, ac: MAX_ARMOR_AC + 1 } }),
      ).success,
    ).toBe(false);
    expect(
      ItemEntry.safeParse(entry(ITEM, { armor: { ...armor, type: "plate" } }))
        .success,
    ).toBe(false);
    expect(
      ItemEntry.safeParse(entry(ITEM, { armor: { ...armor, addDex: "yes" } }))
        .success,
    ).toBe(false);
    expect(
      ItemEntry.safeParse(entry(ITEM, { armor: { type: "light", ac: 12 } }))
        .success,
    ).toBe(false);
  });

  it.each(["1d8", "d6", "2d6", "1d4/1d8", "1d100"])(
    "takes %s as damage",
    (damage) => {
      expect(
        ItemEntry.safeParse(entry(ITEM, { weapon: { ...ITEM.weapon, damage } }))
          .success,
      ).toBe(true);
    },
  );
  ...
  it.each([
    ...
    "a lot",
    "<script>",
  ])("refuses %s as damage — it is a notation, never a formula", (damage) => {
    expect(
      ItemEntry.safeParse(entry(ITEM, { weapon: { ...ITEM.weapon, damage } }))
        .success,
    ).toBe(false);
  ...
  it("keeps weapon properties as tags rather than a sentence", () => {
    ...
    expect(
      ItemEntry.safeParse(
        entry(ITEM, { weapon: { ...weapon, properties: [] } }),
      ).success,
    ).toBe(true);
    expect(
      ItemEntry.safeParse(
        entry(ITEM, { weapon: { ...weapon, properties: ["two-handed"] } }),
      ).success,
    ).toBe(true);
    expect(
      ItemEntry.safeParse(
        entry(ITEM, {
          weapon: { ...weapon, properties: ["Versatile, thrown"] },
        }),
      ).success,
    ).toBe(false);
  });
  ...
  it("matches the weapon type exactly", () => {
    for (const type of ["melee", "ranged", "both"]) {
      expect(
        ItemEntry.safeParse(entry(ITEM, { weapon: { ...ITEM.weapon, type } }))
          .success,
      ).toBe(true);
    }
    expect(
      ItemEntry.safeParse(
        entry(ITEM, { weapon: { ...ITEM.weapon, type: "thrown" } }),
      ).success,
    ).toBe(false);
  });

  it("refuses an unknown key on an item and inside its blocks", () => {
    expect(ItemEntry.safeParse(entry(ITEM, { magical: true })).success).toBe(
      false,
    );
    expect(
      ItemEntry.safeParse(entry(ITEM, { weapon: { ...ITEM.weapon, bonus: 1 } }))
        .success,
    ).toBe(false);
  });
});
...
describe("classes", () => {
  ...
  it.each(["id", "name", "hitDie", "weapons", "armor", "talentTable"])(
    ...
    (field) => {
      expect(
        ClassEntry.safeParse(entry(CLASS, { [field]: undefined })).success,
      ).toBe(false);
    },
  );

  it("takes a single die as the hit die, never a notation", () => {
    expect(ClassEntry.safeParse(entry(CLASS, { hitDie: "d8" })).success).toBe(
      true,
    );
    expect(ClassEntry.safeParse(entry(CLASS, { hitDie: "2d6" })).success).toBe(
      false,
    );
    expect(ClassEntry.safeParse(entry(CLASS, { hitDie: "d7" })).success).toBe(
      false,
    );
    expect(ClassEntry.safeParse(entry(CLASS, { hitDie: 6 })).success).toBe(
      false,
    );
  });
  ...
  it("names weapons bare within a pack and in full across packs", () => {
    expect(
      ClassEntry.safeParse(entry(CLASS, { weapons: ["dagger"] })).success,
    ).toBe(true);
    expect(
      ClassEntry.safeParse(entry(CLASS, { weapons: ["core:item:dagger"] }))
        .success,
    ).toBe(true);
    expect(
      ClassEntry.safeParse(entry(CLASS, { weapons: ["all simple weapons"] }))
        .success,
    ).toBe(false);
    const many = new Array(MAX_TAGS_PER_ENTRY + 1).fill("dagger");
    expect(ClassEntry.safeParse(entry(CLASS, { weapons: many })).success).toBe(
      false,
    );
  });
  ...
  it("lists armour it may wear from the enum and nothing else", () => {
    expect(
      ClassEntry.safeParse(entry(CLASS, { armor: ["none", "light", "shield"] }))
        .success,
    ).toBe(true);
    expect(ClassEntry.safeParse(entry(CLASS, { armor: [] })).success).toBe(
      true,
    );
    expect(
      ClassEntry.safeParse(entry(CLASS, { armor: ["plate"] })).success,
    ).toBe(false);
  });

  it("holds a non-caster as null and a caster as the block", () => {
    expect(
      ClassEntry.safeParse(entry(CLASS, { spellcasting: null })).success,
    ).toBe(true);
    expect(
      ClassEntry.safeParse(entry(CLASS, { spellcasting: undefined })).success,
    ).toBe(true);
    expect(
      ClassEntry.safeParse(
        entry(CLASS, {
          spellcasting: { stat: "int", highestTierByLevel: [1] },
        }),
      ).success,
    ).toBe(true);
  });
  ...
  it("bounds the spellcasting block to one tier per level", () => {
    ...
    expect(
      ClassEntry.safeParse(
        entry(CLASS, {
          spellcasting: { stat: "wis", highestTierByLevel: tiers },
        }),
      ).success,
    ).toBe(false);
    expect(
      ClassEntry.safeParse(
        entry(CLASS, {
          spellcasting: { stat: "wis", highestTierByLevel: [1, 6] },
        }),
      ).success,
    ).toBe(false);
    expect(
      ClassEntry.safeParse(
        entry(CLASS, {
          spellcasting: { stat: "luck", highestTierByLevel: [1] },
        }),
      ).success,
    ).toBe(false);
  });
  ...
  it("names its talent table bare or in full", () => {
    expect(
      ClassEntry.safeParse(
        entry(CLASS, { talentTable: "core:table:wizard-talents" }),
      ).success,
    ).toBe(true);
    expect(
      ClassEntry.safeParse(entry(CLASS, { talentTable: "Wizard talents" }))
        .success,
    ).toBe(false);
  });
  ...
  it("refuses an unknown key on a class", () => {
    expect(
      ClassEntry.safeParse(entry(CLASS, { xpTable: [10, 20] })).success,
    ).toBe(false);
  });
});
...
describe("ancestries", () => {
  ...
  it.each(["id", "name"])("refuses an ancestry missing %s", (field) => {
    expect(
      AncestryEntry.safeParse(entry(ANCESTRY, { [field]: undefined })).success,
    ).toBe(false);
  });

  it("leaves the talent absent or null — core ships without it", () => {
    expect(
      AncestryEntry.safeParse(entry(ANCESTRY, { talent: undefined })).success,
    ).toBe(true);
    expect(
      AncestryEntry.safeParse(entry(ANCESTRY, { talent: null })).success,
    ).toBe(true);
    expect(
      AncestryEntry.safeParse(
        entry(ANCESTRY, { talent: "t".repeat(MAX_TEXT_LENGTH + 1) }),
      ).success,
    ).toBe(false);
  });
  ...
  it("takes a page reference in place of the words", () => {
    expect(
      AncestryEntry.safeParse(entry(ANCESTRY, { talent: null, page: 21 }))
        .success,
    ).toBe(true);
    expect(
      AncestryEntry.safeParse(entry(ANCESTRY, { page: MAX_PAGE_NUMBER + 1 }))
        .success,
    ).toBe(false);
  });
  ...
  it("refuses an unknown key on an ancestry", () => {
    expect(
      AncestryEntry.safeParse(entry(ANCESTRY, { statBonus: "str" })).success,
    ).toBe(false);
  });
});
...
describe("tables", () => {
  ...
  it.each(["id", "name", "die", "rows"])(
    "refuses a table missing %s",
    (field) => {
      expect(
        TableEntry.safeParse(entry(TABLE, { [field]: undefined })).success,
      ).toBe(false);
    },
  );
  ...
  it.each([
    ...
    "d",
    "two d6",
    `${MAX_TABLE_DIE_COUNT + 1}d10`,
    "d20+1",
  ])("refuses %s as a die", (die) => {
  ...
  it("treats an absent rerollable as false rather than refusing the table", () => {
    const absent = TableEntry.safeParse(
      entry(TABLE, { rerollable: undefined }),
    );
    if (!absent.success)
      throw new Error("expected a table with no rerollable to load");
  ...
  it("takes a roll as one face or an inclusive band of them", () => {
    const rows = (roll: unknown): unknown =>
      entry(TABLE, { rows: [{ roll, text: "A result" }] });
  ...
  it("requires every row to say something, and caps how many there are", () => {
    expect(
      TableEntry.safeParse(entry(TABLE, { rows: [{ roll: 1, text: "" }] }))
        .success,
    ).toBe(false);
    expect(
      TableEntry.safeParse(entry(TABLE, { rows: [{ roll: 1 }] })).success,
    ).toBe(false);
    expect(TableEntry.safeParse(entry(TABLE, { rows: [] })).success).toBe(true);

    const rows = new Array(MAX_TABLE_ROWS + 1).fill({
      roll: 1,
      text: "A result",
    });
    expect(TableEntry.safeParse(entry(TABLE, { rows })).success).toBe(false);
    expect(
      TableEntry.safeParse(
        entry(TABLE, { rows: rows.slice(0, MAX_TABLE_ROWS) }),
      ).success,
    ).toBe(true);
  });
  ...
  it("has no grants field on a row, and refuses one", () => {
    ...
    expect(result.success).toBe(false);
    expect(Object.keys(TableEntry.shape.rows.element.shape)).toEqual([
      "roll",
      "text",
    ]);
  });
...
describe("extensions", () => {
  ...
  it("takes a target in the full form only, where no field implies a kind", () => {
    expect(
      PackExtension.safeParse({ target: "core:class:wizard" }).success,
    ).toBe(true);
    expect(PackExtension.safeParse({ target: "core:wizard" }).success).toBe(
      false,
    );
  ...
  it("adds talents in any of the three forms an entry may write", () => {
    const forms = [
      "frost-affinity",
      "frostbound:frost-affinity",
      "frostbound:talent:frost-affinity",
    ];
    expect(
      PackExtension.safeParse({ ...EXTENSION, talents: forms }).success,
    ).toBe(true);
  });

  it("refuses a row an extension adds that a table would have refused", () => {
    const rows = [{ roll: [20, 19], text: "Backwards" }];
    expect(PackExtension.safeParse({ ...ROW_EXTENSION, rows }).success).toBe(
      false,
    );
    expect(
      PackExtension.safeParse({ ...ROW_EXTENSION, rows: [{ roll: 1 }] })
        .success,
    ).toBe(false);
  });
  ...
  it("caps what one extension may add", () => {
    ...
    expect(PackExtension.safeParse({ ...EXTENSION, talents }).success).toBe(
      false,
    );
    expect(PackExtension.safeParse({ ...ROW_EXTENSION, rows }).success).toBe(
      false,
  ...
  it("refuses a field that would edit an entry rather than add to it", () => {
    expect(
      PackExtension.safeParse({ ...EXTENSION, name: "Wizard, better" }).success,
    ).toBe(false);
    expect(
      PackExtension.safeParse({ ...EXTENSION, remove: ["core:spell:light"] })
        .success,
    ).toBe(false);
  });
});
...
describe("an entry inside a pack", () => {
  it("reports its problem with the array, the index and the field", () => {
    const result = parsePack(
      envelope({ spells: [entry(SPELL, { range: "medium" })] }),
    );
    if (result.ok) throw new Error("expected the pack to be refused");
    expect(result.problems.map((problem) => problem.path)).toEqual([
      "spells[0].range",
  ...
  it("lets any entry declare what it overrides, and only in the full form", () => {
    ...
    expect(
      SpellEntry.safeParse(entry(SPELL, { overrides: "fireball" })).success,
    ).toBe(false);
    expect(
      ItemEntry.safeParse(entry(ITEM, { overrides: "core:item:sword" }))
        .success,
    ).toBe(true);
    expect(
      ClassEntry.safeParse(entry(CLASS, { overrides: "core:class:wizard" }))
        .success,
    ).toBe(true);
    expect(
      AncestryEntry.safeParse(
        entry(ANCESTRY, { overrides: "core:ancestry:dwarf" }),
      ).success,
    ).toBe(true);
    expect(
      TableEntry.safeParse(entry(TABLE, { overrides: "core:table:loot" }))
        .success,
    ).toBe(true);
  });
});
...
describe("a reference from one entry to another", () => {
  it("reads all three forms DATA-MODEL.md writes references in", () => {
    ...
    expect(EntryRef.safeParse("core:table:rimewalker-talents").success).toBe(
      true,
    );
  });
