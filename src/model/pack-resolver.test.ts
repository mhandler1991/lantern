...
describe('define', () => {
  ...
  it('lets two packs both define a Skald, and nothing collides', () => {
    ...
    expect(stack.classes.map((one) => one.ref)).toEqual(['north:class:skald', 'south:class:skald']);
    expect(stack.classes.map((one) => one.packName)).toEqual(['The North', 'The South']);
    expect(stack.warnings).toEqual([]);
  });
  ...
  it('warns when two loaded packs share an id, once rather than per entry', () => {
    ...
    expect(stack.warnings.filter((one) => one.path === 'packs[1].id')).toHaveLength(1);
  });
...
describe('override', () => {
  const frostfire = loaded({
    id: 'frostbound',
    name: 'Frostbound',
    spells: [spell('frostfire', 'Frostfire', { overrides: 'core:spell:fireball' })],
  });

  it('replaces by id and warns, because somebody typed the word', () => {
    const stack = resolvePacks([CORE, frostfire]);

    expect(stack.byRef.get('core:spell:fireball')?.entry.name).toBe('Frostfire');
  ...
  it('records both packs against the entry, in load order', () => {
    const sources = resolvePacks([CORE, frostfire]).byRef.get('core:spell:fireball')?.sources;
  ...
  it('gives the last loaded pack the win, and reordering flips it', () => {
    ...
    expect(resolvePacks([CORE, frostfire, cursed]).byRef.get('core:spell:fireball')?.packId).toBe(
      'cursed-scroll',
    );
    expect(resolvePacks([CORE, cursed, frostfire]).byRef.get('core:spell:fireball')?.packId).toBe(
      'frostbound',
    );
  });
...
describe('references', () => {
  it('fills in the segments a pack left implied, from the field they sat in', () => {
    ...
    expect(normalizeRef('core:item:dagger', 'spell', 'frostbound')).toBe('core:item:dagger');
  });

  it('resolves all three forms against the stack', () => {
    const stack = resolvePacks([CORE]);

    expect(lookup(stack, 'dagger', 'item', 'core')?.ref).toBe('core:item:dagger');
    expect(lookup(stack, 'core:dagger', 'item', 'frostbound')?.ref).toBe('core:item:dagger');
    expect(lookup(stack, 'core:item:dagger', 'item', 'frostbound')?.ref).toBe('core:item:dagger');
  });

  it('answers undefined for content no loaded pack defines, and never throws', () => {
    expect(lookup(resolvePacks([CORE]), 'core:item:lantern', 'item', 'core')).toBeUndefined();
  });
  ...
  it('finds a class’s spells from the spells, because that is where the list lives', () => {
    ...
    expect(spellsForClass(stack, 'core:class:wizard').map((one) => one.entry.name)).toEqual([
      'Light',
      'Fireball',
      'Sleep',
      'Hoarfrost',
...
describe('the stack the UI reads', () => {
  ...
  it('reports its warnings in the format a pack’s own problems use', () => {
    ...
    const report = reportProblems(resolvePacks([orphan]).warnings, 'the loaded packs');
