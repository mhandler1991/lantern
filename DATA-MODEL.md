...
| `docs/authoring-prompt.md` | The prompt to paste, the paste-the-errors loop, and what to check before loading | Its enum lists are compared against `model/enums.ts` member by member, and its field census against `model/pack.ts` object by object — a field the page never names is a feature no author can reach |
